...
const intelligenceQueryLimits = {
  maxQueryChars: 2_000,
  maxEntities: 25,
  maxCountries: 25,
  maxPortfolioHoldings: 250,
  maxLookbackChars: 32,
} as const
...
export const marketSnapshotSchema = z.object({
  object: z.literal("market_snapshot").describe("Resource type for a stored latest-market observation."),
  id: z.string().describe("Stable identifier for this stored snapshot."),
  createdAt: z.string().describe("Time this stored snapshot was last written."),
  livemode: z.boolean().describe("Whether the record belongs to a live account context."),
  symbol: symbolSchema.describe("Requested security symbol."),
  name: z.string().nullable().optional().describe("Security name when available."),
  exchange: z.string().nullable().optional().describe("Exchange label supplied with the snapshot when available."),
  currency: z.string().nullable().optional().describe("Currency for price fields when supplied."),
  asOf: z.string().describe("Timestamp represented by the market observation."),
  price: z.number().describe("Latest stored price for the security."),
  change: z.number().nullable().optional().describe("Change from the supplied previous close, when available."),
  changePercent: z.number().nullable().optional().describe("Percent change from the supplied previous close, when available."),
  volume: z.number().int().nonnegative().nullable().optional().describe("Reported trading volume when available."),
  marketCap: z.number().nullable().optional().describe("Reported market capitalization when available."),
  previousClose: z.number().nullable().optional().describe("Previous close used for change fields when available."),
  ...investorMetadataShape,
})

export const marketReferenceSchema = z.object({
  object: z.literal("market_reference").describe("Resource type for security reference metadata."),
  id: z.string().describe("Stable identifier for this reference record."),
  createdAt: z.string().describe("Time this reference record was last written."),
  livemode: z.boolean().describe("Whether the record belongs to a live account context."),
  symbol: symbolSchema.describe("Requested security symbol."),
  name: z.string().nullable().optional().describe("Issuer or security name when available."),
  market: z.string().nullable().optional().describe("Market classification when available."),
  locale: z.string().nullable().optional().describe("Listing locale when available."),
  exchange: z.string().nullable().optional().describe("Primary exchange identifier or label when available."),
  securityType: z.string().nullable().optional().describe("Security type supplied by the reference source when available."),
  active: z.boolean().nullable().optional().describe("Whether the source marks the security active; null means unavailable."),
  currency: z.string().nullable().optional().describe("Listing currency when available."),
  cik: z.string().nullable().optional().describe("SEC CIK when the security is mapped to an SEC registrant."),
  compositeFigi: z.string().nullable().optional().describe("Composite FIGI when available."),
  shareClassFigi: z.string().nullable().optional().describe("Share-class FIGI when available."),
  marketCap: z.number().nullable().optional().describe("Reported market capitalization when available."),
  listDate: z.string().nullable().optional().describe("Listing date when available."),
  ...
  asOf: z.string().describe("Timestamp represented by this reference record."),
  ...investorMetadataShape,
})

export const marketBarSchema = z.object({
  object: z.literal("market_bar").describe("Resource type for one OHLCV price bar."),
  id: z.string().describe("Stable identifier for this bar."),
  symbol: symbolSchema.describe("Security symbol for the bar."),
  interval: z.enum(["1m", "5m", "15m", "1h", "1d", "1w", "1mo"]).describe("Time interval represented by the bar."),
  open: z.number().describe("Opening price for the interval."),
  high: z.number().describe("Highest price for the interval."),
  low: z.number().describe("Lowest price for the interval."),
  close: z.number().describe("Closing price for the interval."),
  volume: z.number().int().nonnegative().nullable().optional().describe("Reported volume for the interval when available."),
  timestamp: z.string().describe("Start timestamp for the bar interval."),
  ...investorMetadataShape,
})

export const corporateActionSchema = z.object({
  object: z.literal("corporate_action").describe("Resource type for a split or dividend event."),
  id: z.string().describe("Stable identifier for the action."),
  symbol: symbolSchema.describe("Security symbol affected by the action."),
  actionType: z.enum(["split", "dividend"]).describe("Corporate-action category."),
  exDate: z.string().describe("Ex-date for the action."),
  recordDate: z.string().nullable().optional().describe("Record date when available."),
  payDate: z.string().nullable().optional().describe("Payment date when available."),
  ratio: z.number().nullable().optional().describe("Split ratio when applicable."),
  cashAmount: z.number().nullable().optional().describe("Cash amount per share for a dividend when available."),
  currency: z.string().nullable().optional().describe("Currency for cashAmount when available."),
  ...investorMetadataShape,
})
...
export const macroStatusSchema = z.object({
  ...
  artifactGeneratedAt: z.string().nullable().optional(),
  computedAt: z.string().optional(),
  statusSource: z.enum(["validation_artifacts", "runtime_registry_fallback"]).optional(),
  ...
  sources: z.array(z.object({
    ...
    sourceErrorCount: z.number().int().nonnegative(),
    artifactStatus: z.enum(["available", "unavailable"]).optional(),
    topAlerts: z.array(z.string()).default([]).optional(),
  })).default([]),
...
export const intelligenceQueryRequestSchema = z.object({
  query: z.string().min(1).max(intelligenceQueryLimits.maxQueryChars),
  entities: z.array(symbolSchema).max(intelligenceQueryLimits.maxEntities).default([]),
  portfolio: z.array(portfolioHoldingInputSchema).max(intelligenceQueryLimits.maxPortfolioHoldings).nullable().optional(),
  countries: z.array(isoCountryCodeSchema).max(intelligenceQueryLimits.maxCountries).default([]),
  lookback: z.string().max(intelligenceQueryLimits.maxLookbackChars).nullable().optional(),
